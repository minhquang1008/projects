...
, [WorkDays] AS (
	SELECT
		...
		, CASE 
			...
			ELSE DATETIMEFROMPARTS(YEAR([Date]),2,28,0,0,0,0)
		END [EndOfPeriod]
	FROM [Date]
	WHERE [Work] = 1
		AND [Date] BETWEEN @Since AND @Date
		AND DAY([Date]) <= DAY(@Date) AND MONTH([Date]) <= MONTH(@Date)
)
...
, [ValueByBranchByDate] AS (
	...
	WHERE [trading_record].[date] BETWEEN @Since AND @Date
		AND DAY([trading_record].[date]) <= DAY(@Date) AND MONTH([trading_record].[date]) <= MONTH(@Date)
