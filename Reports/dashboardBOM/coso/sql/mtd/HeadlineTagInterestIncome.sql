...
[BadDebt] AS (
	...
	WHERE [Ngay] BETWEEN DATEADD(MONTH, -1, @FirstDateOfMonthOfPreviousYear) AND @Date
...
, [PrevValuePeriod] AS (
	...
	WHERE [rln0019].[date] BETWEEN @FirstDateOfMonthOfPreviousYear AND @DateOfPreviousYear
...
, [CurrentValuePeriod] AS (
	...
	LEFT JOIN [BadDebt]
		ON [BadDebt].[SoTaiKhoan] = [relationship].[account_code]
		AND [BadDebt].[Ngay] = CONVERT(VARCHAR(7), DATEADD(MONTH, -1, [rln0019].[date]), 126)
	WHERE [rln0019].[date] BETWEEN @FirstDateOfMonthOfCurrentYear AND @Date
		AND [relationship].[branch_id] IN (SELECT [BranchID] FROM [TargetByBranch])
)
