...
, [RRE0386.Flex] AS (
	...
	FROM [RRE0386]
	WHERE (
		([RRE0386].[NgayMoTK] BETWEEN @FirstDateOfMonthOfPreviousYear AND @DateOfPreviousYear)
		OR ([RRE0386].[NgayMoTK] BETWEEN @FirstDateOfMonthOfCurrentYear AND @Date)
	)
...
, [FindLastBranchID] AS (
	...
	FROM [relationship]
	WHERE (
		([date] BETWEEN @FirstDateOfMonthOfPreviousYear AND @DateOfPreviousYear)
...
, [ValueByPrevPeriod] AS (
	...
	WHERE [Rel].[BranchID] IN (SELECT [BranchID] FROM [TargetByBranch])
		AND [_RRE0386].[NgayMoTK] BETWEEN @FirstDateOfMonthOfPreviousYear AND @DateOfPreviousYear
)
...
, [ValueCurrentPeriod] AS (
	...
	LEFT JOIN [Rel]
		...
		AND [Rel].[Date] = [_RRE0386].[NgayMoTK]
	WHERE [Rel].[BranchID] IN (SELECT [BranchID] FROM [TargetByBranch])
		AND [_RRE0386].[NgayMoTK] BETWEEN @FirstDateOfMonthOfCurrentYear AND @Date
)
